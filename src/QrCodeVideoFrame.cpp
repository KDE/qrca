...
uchar yuvToGray(uchar Y, uchar U, uchar V)
{
    ...
    return gray(qBound<uchar>(0, uchar((298 * C + 409 * E + 128) >> 8), 255), qBound<uchar>(0, uchar((298 * C - 100 * D - 208 * E + 128) >> 8), 255), qBound<uchar>(0, uchar((298 * C + 516 * D + 128) >> 8), 255));
}
...
uchar yuvToGray2(uchar y, uchar u, uchar v)
{
    ...
    return gray(qBound<uchar>(0, uchar(floor(rD)), 255), qBound<uchar>(0, uchar(floor(gD)), 255), qBound<uchar>(0, uchar(floor(bD)), 255));
}

static QImage *rgbDataToGrayscale(const uchar *data, const CaptureRect &captureRect, const int alpha, const int red, const int green, const int blue, const bool isPremultiplied = false)
{
